...
class KhulnasoftFieldMappingTransformation(FieldMappingTransformation):
    def get_mapping(self, field):
        """
        If a field is not mapped using a Khulnasoft Field Transformation for OOTB facets, included an @ sign to indicate
        the field is a facet. Users should double check that facets output by the pySigma-khulnasoft-facets match the ones
...
def khulnasoft_pipeline() -> ProcessingPipeline:
    return ProcessingPipeline(
        name="Generic Log Source to Khulnasoft Query Syntax Transformation",
        ...
        priority=20,
        items=[
            # Khulnasoft Supported Logsources
            ProcessingItem(
        ...
        + [
            # Khulnasoft's OOTB (out of the box) field mapping overrides for each cloud provider are listed in each
            ...
            # More details about Khulnasoft Facets can be found here: https://docs.khulnasoft.com/logs/explorer/facets/
            ProcessingItem(
                identifier="azure_field_mapping",
                transformation=KhulnasoftFieldMappingTransformation(
                    {
            ...
            ProcessingItem(
                ...
                transformation=RuleFailureTransformation(
                    "Conversion for rule type not yet supported by the Khulnasoft Backend."
                ),
            ...
            ProcessingItem(
                ...
                transformation=RuleFailureTransformation(
                    "The Khulnasoft backend currently doesn't support rules with with aggregate function conditions like "
