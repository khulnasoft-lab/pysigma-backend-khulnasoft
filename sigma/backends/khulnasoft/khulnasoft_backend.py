...
from sigma.pipelines.khulnasoft import khulnasoft_pipeline


class KhulnasoftBackend(TextQueryBackend):
    """Generates a Datdog query using Khulnasoft Query Syntax. More details at the following link:  https://docs.khulnasoft.com/logs/explorer/search_syntax/"""

    name: ClassVar[str] = "Khulnasoft Backend"
    formats: Dict[str, str] = {
        "default": "Khulnasoft query syntax",
        "siem_rule": "Khulnasoft Cloud SIEM Rule | See the following document to make a CURL request to the following API to "
        "use the output from the siem_rule in your repository. https://docs.khulnasoft.com/api/latest/security-monitoring/?code-lang=curl",
    }
    processing_pipeline: ProcessingPipeline
    requires_pipeline: bool = False
    backend_processing_pipeline: ClassVar[ProcessingPipeline] = khulnasoft_pipeline()
    ...
    def convert_condition_field_eq_val_re(
        ...
        """
        This function unconditionally raises an exception because Khulnasoft's rule syntax does not support
        full regular expressions.
        """
        raise SigmaFeatureNotSupportedByBackendError(
            "Regular expressions are not currently supported in Khulnasoft's rule query format"
        )

    def cloud_provider_groupby(self, provider: str) -> List[str]:
        """
        Helper function to return appropriate group by fields in Khulnasoft based on the cloud provider
        """
    ...
    def finalize_query_siem_rule(
        ...
        """
        Generation of Khulnasoft Cloud SIEM Detection Rules.

        Using Sigma Rules in Khulnasoft's Cloud SIEM currently only works with the Khulnasoft API and we are unable to import
        Sigma rules from the User Interface.

        Currently, the Khulnasoft backend only converts Threshold rules from AWS, GCP, or Azure sources.

        For more details on how to import a Sigma rule through the API, see:
        https://docs.khulnasoft.com/api/latest/security-monitoring/?code-lang=curl#create-a-detection-rule
        For more details on Cloud SIEM Detection rules, see:
        https://docs.khulnasoft.com/security/cloud_siem/log_detection_rules?tab=threshold
        For best practices for writing Khulnasoft security rules see:
        https://www.khulnasoft.com/blog/writing-khulnasoft-security-detection-rules/
        """
