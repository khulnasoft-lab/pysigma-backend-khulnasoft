...
def test_aws_field_transformations():
    assert (
        KhulnasoftBackend().convert(
            SigmaCollection.from_yaml(
                """
                title: Khulnasoft AWS Field Transformation Test
                status: test
                logsource:
                    product: aws
                    service: cloudtrail
...
def test_khulnasoft_azure_transformations_test():
    assert (
        KhulnasoftBackend().convert(
            SigmaCollection.from_yaml(
                """
...
def test_khulnasoft_multiple_evt_names_aws():
    assert (
        KhulnasoftBackend().convert(
            SigmaCollection.from_yaml(
                """
...
def test_khulnasoft_unsupported_rule_type():
    with pytest.raises(
        SigmaTransformationError,
        match="Conversion for rule type not yet supported by the Khulnasoft Backend.",
    ):
        KhulnasoftBackend().convert(
            SigmaCollection.from_yaml(
                """
...
def test_khulnasoft_pipeline_unsupported_aggregate_conditions_rule_type():
    with pytest.raises(
        SigmaTransformationError,
        match="The Khulnasoft backend currently doesn't support rules with with aggregate function conditions like count, min, max, avg, sum, and near.",
    ):
        KhulnasoftBackend().convert(
            SigmaCollection.from_yaml(
                """
...
def test_khulnasoft_pipeline_multiple_filters():
    assert (
        KhulnasoftBackend().convert(
            SigmaCollection.from_yaml(
                """
...
def test_khulnasoft_pipeline_unsupported_regex():
